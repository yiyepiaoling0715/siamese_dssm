...
class InputHelper():
    ...
    def __init__(self, data_dir, input_file, batch_size, sequence_length, split_ratio, is_train=True):
        ...
        xs= self.create_batches(input_file)
        self.raw_x1=xs
        self.x1_train, self.x1_test= self.split_corpus(xs)
    ...
    def split_corpus(self, x1):
        return train_test_split(x1, train_size=self.split_ratio)
        ...
        with open(text_file, 'r', encoding='utf-8') as fr:
            for line in fr:
                ...
                for x_iter in xs:
                    ...
                    xs_padding.append(ids_x1_iter_padding)
                    # x2.append(ids_x2_iter_padding)
                    # y.append(id_y_iter)
                datas.append(xs_padding)

        x1_array = np.array(datas)
        return x1_array
    ...
    def next_batch(self, train_or_test, random=True):
        ...
        for batch_index in range(int(batch_number)):
            ...
            # x1_iter.split()
            x_pos,x_neg=np.split(x1_iter,[1],axis=1)
